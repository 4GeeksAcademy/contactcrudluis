...
export const AddContact = () => {
	...
	useEffect(() => {
		if (id && store && store.contacts && store.contacts.length > 0) {
		...
		<div className="container mt-5" style={{ maxWidth: "700px" }}>
			...
			<form onSubmit={handleSubmit}>
				...
				<div className="mb-3">
					...
					<input type="text" className="form-control" name="phone" placeholder="Enter phone" value={formData.phone} onChange={handleChange} required />
				</div>
				<div className="mb-3">
					<label className="form-label fw-bold">Address</label>
					<input type="text" className="form-control" name="address" placeholder="Enter address" value={formData.address} onChange={handleChange} required />
				</div>
				
				<button type="submit" className="btn btn-primary w-100 mb-3">save</button>
				<Link to="/" className="d-block text-center text-decoration-none">or get back to contacts</Link>
			</form>
		</div>
	);
};